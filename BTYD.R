...
elog=read.csv("behaviour_north_metal.csv")

elog = elog[elog$Date<=20171206,]   # read data

str(elog)

head(elog)                              # take a look

elog<-elog[,-1]                   # we need these columns
...
purchaseFreq <- ddply(elog2, .(cust), summarize, 
...
(end.of.cal.period <-
   min(elog$date)+as.numeric((max(elog$date)-min(elog$date))/1))

(max(elog$date)-as.Date("2016-08-03"))/7
...
pnbd.Expectation(params2, t = 104)
...
cal2.cbs["101",]
x <- cal2.cbs["101", "x"]         # x is frequency
t.x <- cal2.cbs["101", "t.x"]     # t.x is recency, ie time of last transactions
T.cal <- cal2.cbs["101", "T.cal"] # T.cal is total time observed

# estimate transactions in a T.star-long duration for that cust
pnbd.ConditionalExpectedTransactions(params2, T.star = 104, # weeks
                                     x, t.x, T.cal)
...
T.cal <- 70 # week of end of cal, i.e. present
...
predict<-pnbd.ConditionalExpectedTransactions(params2, T.star = 34, # weeks
                                              x     = cal2.cbs[,"x"], 
                                              t.x   = cal2.cbs[,"t.x"], 
                                              T.cal = cal2.cbs[,"T.cal"])
